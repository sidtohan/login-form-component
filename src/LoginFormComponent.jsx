...
const LoginFormComponent = ({}) => {
  ...
  const handleLogin = (e) => {
    ...
    axios
      .post("https://adding-nfts.herokuapp.com/login-user", {
        email: email,
        password: password,
      })
      .then((response) => {
  ...
  const connectWallet = async (e) => {
    ...
    axios
      .post(
        "https://adding-nfts.herokuapp.com/update-user-wallet",
        { email: email, newWallet: accounts[0] },
        config
      )
      .then((response) => {
  ...
  return (
    <section className="login-form">
      <img className="login-logo" src={logo} alt="Griffin Art" />
      {!loggedIn ? (
        <>
          <h3>Login</h3>
